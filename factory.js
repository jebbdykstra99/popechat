(function () {
  ...
  let SITE_ID = 'popechat';
  let site = null;
  let COLORS = ['#3b1848', '#c4a056', '#9a7c3a', '#6b4a63', '#5a2868', '#2a1028'];
  let TRENDS = [];
  let PLACES = [];
  let TOPICS = [];
  ...
  function syncProfile() {
    ...
    document.getElementById('profile-bio').textContent = currentUser.bio || "Urbi et orbi.";
    ...
    if (!pane) return;
    if (!mine.length) {
      pane.innerHTML = '<div class="empty-note" id="profile-posts-empty">No posts yet. Speak when you have something for the square.</div>';
    } else {
  ...
  function wireEvents() {
    ...
    search.addEventListener('input', function () {
      ...
      function cards(list) {
        if (!list.length) return '<p class="empty-note">Nothing in the square matched that.</p>';
        return list.map(function (c) {
  ...
  fetch(SITE_JSON_URL)
    ...
    .catch(function (e) {
      ...
      boot({ siteId: "popechat", name: "popechat", tagline: "Urbi et orbi." });
    });
})();
